...
\begin{defn}
    ...
    \mathcal{X}(G) = \min \{k ~|~ \exists \text{ a }k\text{-coloring of }G\}.
    \]
\end{defn}
\begin{example}
    \begin{itemize}
        \item A path has chromatic number 2. $\mathcal{X}(P_n)=2$.
        \item For a cycle, $\mathcal{X}(G) = \begin{cases}
            2 &\text{ if }n \text{ is even.}\\
            3 &\text{ if }n \text{ is odd.}
        \end{cases}$
        \item A tree has chromatic number 2 by induction.
        \item A complete graph has $\mathcal{X}(K_n) = n$.
        \item A bipartite graph has $\mathcal{X}(K_{m,n}) = 2$. In fact, a graph $G$ is bipartite if and only if $\mathcal{X}(G)=2$.
    \end{itemize}
\end{example}
\begin{prop}
    Let $G$ be a graph. Then $\mathcal{X}(G) \le \Delta(G) + 1$.
\end{prop}
\begin{proof}
...
\end{proof}
\begin{prop}
    If $G$ is planar, then $\mathcal{X}(G)\le 6$.
\end{prop}
\begin{proof}
...
\begin{theorem}
    If $G$ is planar, then $\mathcal{X}(G) \le 5$. 
\end{theorem}
\begin{proof}
...
\begin{theorem}[Four color theorem, non-examinable]
    If $G$ is planar, then $\mathcal{X}(G)\le 4$.
\end{theorem}
...
\begin{prop}\label{3.5}
    Let $G$ be connected and $\delta(G)<\Delta(G)$. Then $\mathcal{X}(G)\le \Delta(G)$.
\end{prop}
\begin{proof}
...
\begin{theorem}[Brooks]
    Let $G$ be a connected graph which is not an odd cycle or a complete graph. Then $\mathcal{X}(G)\le \Delta(G)$.
\end{theorem}
\begin{proof}
...
\begin{defn}
    ...
    In particular, the minimal value of $t$ for which $P_G(t)>0$ is $t = \mathcal{X}(G)$.
\end{defn}
\begin{example}
    \begin{itemize}
...
\begin{itemize}
    \item Other coefficients of $P_G$ contain other information about $G$, e.g. $$P_G = t^n - mt^{n-1} + \left({{n}\choose{2}} - \text{the number of triangles in }G\right)t^{n-2} + \ldots.$$
...
\begin{defn}
    ...
    Define the \textbf{edge-chromatic number} (sometimes called the \textbf{chromatic index}) to be $$\mathcal{X}'(G) = \min\{k \mid \exists k\text{-edge coloring of }G\}.$$
\end{defn}
Note that an edge coloring of $G$ corresponds exactly to a vertex coloring of the line graph of $G$, and so $\mathcal{X}'(G)=\mathcal{X}(L(G))$.
\vspace{1mm}

\textbf{Remarks.} \begin{itemize}
    \item $\mathcal{X}'(G) = \begin{cases}
        2 &\text{ if } n \text{ even.} \\
        3 &\text{ if } n \text{ odd.}
    \end{cases}$
    \item $\Delta(G) \le \mathcal{X}'(G)$.
    \item We can have $\Delta(G) < \mathcal{X}'(G)$. For example, $\mathcal{X}'(\text{Petersen})=4$.
    \item $\mathcal{X}'(G)\le 2 \Delta(G)-1$ by coloring greedily.
    \item $\mathcal{X}$ and $\mathcal{X}'$ can be very different. For example, $\mathcal{X}'(K_{t,1})=t, \mathcal{X}(K_{t,1})=2$.
\end{itemize}
...
\begin{theorem}[Vizing's theorem]
    Let $G$ be a graph. Then $$\mathcal{X}'(G) \in \{\Delta(G), \Delta(G)+1\}.$$
\end{theorem}
\begin{proof}
