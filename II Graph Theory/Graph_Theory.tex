...
*picture*
$K_7$ can be drawn on a Torus without edge crossings, so we don't have a 4 color theorem for the torus. 

\textbf{Question.} Is there a "7 color theorem" for the torus?

\begin{defn}
    We define the surface of genus $g$ (officialy a compact orientable surface of genus $g$) to be the sphere with $g$ handles attached (or equivalently, the sphere with $g$ holes).
\end{defn}
...
\begin{theorem}[Dirac]
    Let $G$ be a graph, $|G|\ge 2$. If $\delta(G)\ge \frac{|G|}{2}$, then $G$ contains a cycle of length $|G|$.
\end{theorem}
